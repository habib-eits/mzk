...
$total_present = \App\Models\Attendance::selectRaw('EmployeeID, MonthName, 
...
$overtime = \App\Models\OverTime::where('MonthName',request()->MonthName)->where('EmployeeID',$value->EmployeeID)->get();
...
$advance = \App\Models\Journal::whereRaw(
    ...
    ->selectRaw("COALESCE(SUM(Dr), 0) - COALESCE(SUM(Cr), 0) AS AdvanceSalary")
    ->value('AdvanceSalary');

$sf = \App\Models\SupervisorFine::where('MonthName',request()->MonthName)->where('EmployeeID',$value->EmployeeID)->get();

$SD = $sf->sum('Amount');


$commission = \App\Models\SupervisorFine::where('MonthName',request()->MonthName)->where('SupervisorEmployeeID',$value->EmployeeID)->get();
...
$visa_deduction = \App\Models\Journal::where('EmployeeID', $value->EmployeeID)
    ->where('ChartOfAccountID', 210101) // Visa Charges Recoverable from Staff
    ->selectRaw("COALESCE(SUM(Dr), 0) - COALESCE(SUM(Cr), 0) AS visa_deduction")
...
$salary_deduction =  \App\Models\Journal::where('EmployeeID', $value->EmployeeID)
    ->where('ChartOfAccountID', 112309) // Staff Salary Security Deposit
    ->selectRaw("COALESCE(SUM(Cr), 0) - COALESCE(SUM(Dr), 0) AS salary_deduction")
    ->value('salary_deduction');


$training_deduction =  \App\Models\Journal::where('EmployeeID', $value->EmployeeID)
    ->where('ChartOfAccountID', 570101) // 570101 => staff training expenses
    ->selectRaw("COALESCE(SUM(Dr), 0) - COALESCE(SUM(Cr), 0) AS salary_deduction")
    ->value('salary_deduction');
...
<input type="hidden" name="StaffType" value=" ">
        ...
        <td ><input type="text" size="2" name="DaysPresent[]" id="DaysPresent_{{$no}}" value="{{$total_present->TotalAttendance ?? 0}}" class="changesNo"  ></td>

         <td ><input type="number" step="0.01" size="2" name="PerDay[]" id="PerDay_{{$no}}" value="{{$perDay}}" class="changesNo" style="width: 80px;"></td>

        <td ><input type="number" step="0.01" size="2" name="Salary[]" id="Salary_{{$no}}" value="{{$salary}}" class="changesNo" style="width: 80px;" readonly="" ></td>


        <td ><input type="number" step="0.01" size="2" name="OT[]" id="OT_{{$no}}" value="{{$overtime->sum('Total')}}" class="changesNo" style="width: 80px;"></td>
        
        
        <td ><input type="number" step="0.01" size="2" name="Commission[]" id="Commission_{{$no}}" value="{{$commission->sum('ComissionAmount')}}" class="changesNo" style="width: 80px;"></td>

        <td ><input type="number" step="0.01" size="2" name="Advance[]" id="Advance_{{$no}}" value="{{$advance}}" class="changesNo" style="width: 80px;"></td>
        <td ><input type="number" step="0.01" size="2" name="Visa_deduction[]" id="VisaDeduction_{{$no}}" value="{{$visa_deduction}}" class="changesNo" style="width: 80px;"></td>

        <td ><input type="number" step="0.01" size="2" name="SD[]" id="SD_{{$no}}" value="{{$SD}}" class="changesNo" style="width: 80px;"></td>

        <td ><input type="number" step="0.01" size="2" name="training_deduction[]" id="trainingDeduction_{{$no}}" value="{{$training_deduction}}" class="changesNo" style="width: 80px;"></td>
        
        <td ><input type="number" step="0.01" size="2" name="salary_deduction[]" id="salaryDeduction_{{$no}}" value="{{$salary_deduction}}" class="changesNo" style="width: 80px;"></td>

        <td ><input type="number" step="0.01" size="2" name="NetSalary[]" id="NetSalary_{{$no}}" value="{{$netSalary}}"  style="width: 80px;" readonly=""></td>
      
        <td ><input type="text" name="Notes[]" id="Notes_{{$no}}" style="width: 100%;"></td>
