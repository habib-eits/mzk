                        ...
                        <div class="card-body">
                                        ...
                                        <div class="col-sm-9">
                                            <select name="PartyID" id="PartyID" class="form-select select2 mt-5"
                                                ...
                                                <option value="{{ $value->PartyID }}"
                                                    ...
                                                    {{ $value->PartyName }}</option> <?php endforeach ?>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-1 row">
                                        ...
                                        <div class="col-sm-9">
                                            <select name="JobID" id="JobID" class="form-select select2 mt-5"
                                                ...
                                                <option value="{{ $value->JobID }}"
                                                    {{ $value->JobID == $invoice_master[0]->JobID ? 'selected=selected' : '' }}>
                                                    {{ $value->JobNo }}</option> <?php endforeach ?>
                                            </select>
                                        </div>
                                    </div>
                                     ...
                                     <div class="mb-1 row">
                                        ...
                                        <div class="col-sm-9">
                                            <select name="PONo" id="PONo" class="form-select select2 mt-5"    style="width:100%;">
                                                <option value="">Select</option>
                                                <?php foreach ($po as $key => $row) : ?>
                                                    <option value="{{$row->InvoiceNo}}"  {{ $row->InvoiceNo == $invoice_master[0]->PONo ? 'selected=selected' : '' }}  >{{$row->InvoiceNo}}</option> <?php endforeach ?>
                                            </select>
                                        </div>
                                    </div>
                                            ...
                                            <div class="col-sm-9">
                                                <select name="BranchID" id="BranchID" class="form-select">
                                                    ...
                                                    <option value="{{ $value->BranchID }}"
                                                        {{ $value->BranchID == $invoice_master[0]->BranchID ? 'selected=selected' : '' }}>
                                            ...
                                            @foreach ($invoice_detail as $value1)
                                                <tr class="p-3">
                                                    ...
                                                    <td valign="top">
                                                        ...
                                                        <select name="ItemID0[]" id="ItemID0_{{ $loop->iteration }}"
                                                    ...
                                                    <td valign="top"><select name="UnitName[]"
                                                            id="UnitName_{{ $loop->iteration }}" class="form-select">
                                            ...
                                            <label class="col-form-label" for="password">Salesperson </label>
                                                                ...
                                                                <div class="col-sm-9 input-group">
                                                                    <select name="DiscountType[]"
                                                                        id="DiscountType_{{ $loop->iteration }}"
                                                                    ...
                                                                    <input type="hidden" name="DiscountAmountItem[]"
                                                                        id="DiscountAmount_{{ $loop->iteration }}"
                                                                        value="0">
                                                                </div>
                                                            </span>
                                                    ...
                                                    <td valign="top">
                                                        <select name="Tax[]" id="TaxID_{{ $loop->iteration }}"
                                                            required=""
                            ...
                            <div class="row mt-4">
                                ...
                                <div class="col-lg-8 col-12  ">
                                    ...
                                    <textarea class="form-control kashif" rows='5' name="CustomerNotes" id="note" placeholder=""> {{ $invoice_master[0]->CustomerNotes }}</textarea>

                                    <label for="" class="mt-2">Description</label>
                                    <textarea class="form-control kashif " rows='5' name="DescriptionNotes" id="note"> {{ $invoice_master[0]->DescriptionNotes }}</textarea>
    ...
    <script>
        ...
        $(".addmore").on('click', function() {
            ...
            html += '<td><select name="ItemID0[]" id="ItemID0_' + i +
                ...
                i +
                '"> <textarea name="Description[]"  rows="2" class="form-control  mt-1 tiny-editor" style="width: 100% !important;"></textarea></td>';
...
$(document).ready(function () {
    ...
    function loadJobsByParty(partyID) {
        $.ajax({
            type: 'POST',
            url: '{{ route("ajax.job2") }}',
            data: { PartyID: partyID, _token: $('#csrf').val() },
            dataType: 'json',
            beforeSend: function () {
            ...
            success: function (response) {
                $('#JobID').html(response.options); // populate Job dropdown
                $('#fupForm').css("opacity", "");
    ...
    // Call function when PartyID changes
    $(document).on('change', '#PartyID', function () {
        let selectedValue = $(this).val();
        loadJobsByParty(selectedValue); // ✅ function call
    });

    // Example: you can call it somewhere else too
    // loadJobsByParty(101); // manually load jobs for PartyID=101
});
   ...
   <script>
    $(document).ready(function () {
                                 ...
                                 tinymce.init({
                                    ...
                                    selector: ".kashif", // Select all textarea exluding the mceNoEditor class
                                    height: 200,
                                    menubar: false,
                                    plugins: [
                                        'advlist autolink lists link image charmap print preview anchor textcolor',
