                                                ...
                                                <select name="UserID" id="UserID" class="form-select">
                                                    ...
                                                    @foreach ($users as $value)
                                                        <option value="{{ $value->UserID }}"
                                                            {{ old('UserID') == $value->UserID ? 'selected=selected' : '' }}>
                                            ...
                                            <div class="col-sm-2 mt-2 mb-3">
                                                ...
                                                <div class="custom-control custom-checkbox">
                                                    <input name="Check[]" type="checkbox" class="custom-control-input"
                                                        id="{{ $value->RoleID }}" value="Y"
                                                        {{ $result != 0 ? 'checked' : '' }}>
                                                    <label class="custom-control-label"
                                                        for="customCheck33079">{{ $value->Action }} </label>
                                                    <label>
                                                        <input name="TableName[]" type="hidden" id="{{ $value->RoleID }}"
                                                            value="{{ $value1->Table }}">
