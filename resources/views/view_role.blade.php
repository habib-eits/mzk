...
$result=check_role($users[0]->UserID,$value->Table,$value->Action);
  ...
  <label>
  <input name="TableName[]" type="text" id="{{$value->RoleID}}" value="{{$value1->Table}}">
   <input name="Action[]" type="text" id="{{$value->RoleID}}" value="{{$value->Action}}">
   <input name="Allow[]" type="text" id="{{$value->RoleID}}Allow" value="{{$result[0]->Allow}}" class="role" >
