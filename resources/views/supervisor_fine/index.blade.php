                        ...
                        <form id="create-update-form" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-1">
                                    ...
                                    <select name="SupervisorEmployeeID" id="SupervisorEmployeeID" class="form-select select2 " style="width: 100%;" >
                                        @foreach ($employee as $row)
                                        <option value="{{$row->EmployeeID}}">{{$row->FirstName}} {{$row->MiddleName}} {{$row->LastName}}</option>
                                ...
                                <div class="col-md-2 mb-1">
                                    <label class="form-label">Percentage 20%</label>
                                    <input type="decimal" name="Percentage"  class="form-control" id="Percentage"  value="20" readonly>
                                </div>
                                
                                
                                <div class="col-md-2 mb-1">
                                    <label class="form-label">Commission</label>
                                    <input type="decimal" name="ComissionAmount"  class="form-control" id="ComissionAmount" readonly>
                                </div>
                                
                                <div class="col-md-6 mb-1">
                                    <label class="form-label">Reason for Fine</label>
    ...
    <script>
        ...
        $(document).ready(function() {
            table = $('#table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('supervisor.fine.index') }}",
                columns: [
            ...
            $('#create-update-form').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('supervisor.fine.store') }}",
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    cache: false,
        ...
        function editRecord(id) {
            var btn = $('#submit-btn');
            $.get("{{ route('supervisor.fine.edit', ':id') }}".replace(':id', id), function(response) {
                ...
                $('#SupervisorEmployeeID').val(response.data.SupervisorEmployeeID);
                $('#Amount').val(response.data.Amount);
                $('#Percentage').val(response.data.Percentage);
                $('#ComissionAmount').val(response.data.ComissionAmount);
        ...
        function deleteRecord(id) {
            if (confirm("Are you sure you want to delete?")) {
                $.ajax({
                    type: 'DELETE',
                    url: "{{ route('supervisor.fine.destroy', ':id') }}".replace(':id', id),
                    data: {
...
<script>
    $(document).ready(function () {
        function calculateCommission() {
            ...
            $('#ComissionAmount').val(commission.toFixed(2)); // rounded to 2 decimals
        }

        $('#Amount').on('input', calculateCommission);
    });
</script>
